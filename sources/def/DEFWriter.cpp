...
void def::DEFWriter::OutDEF(std::string nameOutFile, DEF_File& def) {
    ...
    outFile << "###############################################################" << std::endl;
    outFile << "#  Generated by:      Cadence Innovus 21.38-e039_1" << std::endl;
    outFile << "#  OS:                Linux x86_64(Host ID rno-sim-a07-170)" << std::endl;
    outFile << "#  Generated on:      Mon May  5 12:17:19 2025" << std::endl;
    outFile << "#  Design:            aes_cipher_top" << std::endl;
    outFile << "#  Command:           defOut -netlist aes_cipher_top.def" << std::endl;
    outFile << "###############################################################" << std::endl;
    ...
    outFile << "BUSBITCHARS \"" << def.BUSBITCHARS[0] << def.BUSBITCHARS[1] << "\" ;" << std::endl;
    outFile << "DESIGN " << def.DESIGN <<   " ;" << std::endl;
    outFile << "UNITS DISTANCE MICRONS " << def.UNITS_DISTANCE_MICRONS << " ;" << std::endl << std::endl;
    outFile << "PROPERTYDEFINITIONS" << std::endl;
    for (int i = 0; i < (def.PROPERTYDEFINITIONS.size()); i++) {
        outFile << "\t" << def.PROPERTYDEFINITIONS[i] << std::endl;
    }
    outFile << "END PROPERTYDEFINITIONS" << std::endl << std::endl;
    outFile << "DIEAREA ( " << def.DIEAREA.x1 << " " << def.DIEAREA.y1 << " ) ( " 
            << def.DIEAREA.x2 << " " << def.DIEAREA.y2 << " ) ;" << std::endl << std::endl;
